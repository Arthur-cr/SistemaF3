...
namespace Domain.Servicos
{
    public class DespesaServico : IDespesaServico
    {
        ...
        public async Task<object> CarregaGraficos(string emailUsuario)
        {
            ...
            var despesas_pagas = despesaUsuario.Where(d => d.Pago && d.TipoDespesa == Entities.Enums.EnumTipoDespesa.Contas)
                .Sum(x => x.Valor);

            var despesas_pendentes = despesaUsuario.Where(d => !d.Pago && d.TipoDespesa == Entities.Enums.EnumTipoDespesa.Contas)
                .Sum(x => x.Valor);

            var investimentos = despesaUsuario.Where(d => d.TipoDespesa == Entities.Enums.EnumTipoDespesa.Investimento)
                .Sum(x => x.Valor);
